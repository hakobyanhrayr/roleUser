                            ...
                            <form action="{{route('roles.update',$role->id)}}" method="POST">
                                ...
                                @foreach($permissions as $permission)
                                    <div class="form-group form-check">
                                        <input type="checkbox" value="{{$permission->id}}" name="permissions[]" @if($role->hasPermissionTo($permission->name)) checked @endif class="form-check-input" id="exampleCheck{{$permission->id}}">
